...
from . lib import client, saas_client_db
...
class SaasClient(models.Model):
    ...
    def fetch_client_url(self, domain_name=None):
        for obj in self:
            ...
            if response:
                ...
                modules_list = [module.technical_name for module in obj.saas_module_ids]
                url = "http://localhost:{}".format(obj.containter_port)

                client = saas_client_db.connect_db(url, obj.database_name, "admin", "admin", flag=False)
                if not client:
                    response['modules_installation'] = False
                    return response
                response['modules_installation'], response['modules_missed'] = saas_client_db.install_modules(client, modules_list)
    ...
    def login_to_client_instance(self):
        for obj in self:
            host_server, db_server = obj.saas_contract_id.plan_id.server_id.get_server_details()
            _logger.info("+++++++++++++++++%r+++++++++%r"%(host_server, db_server))
    ...
    def stop_client(self):
        for obj in self:
            host_server, db_server = obj.saas_contract_id.plan_id.server_id.get_server_details()
            response_flag = containers.action(operation="stop",container_id=obj.container_name,host_server = host_server,db_server = db_server)
            if response_flag:
                obj.state = "stopped"
            else:
    ...
    def disable_client_wizard(self):
        ...
        res_wizard = self.env['saas.client.disable'].sudo().create({'client_id': self.id})
        return{
            'name' : 'Disable Client',
            'view_mode' : 'form',
    ...
    def unlink(self):
        for obj in self:
            if obj.state == "started":
                raise UserError("You cannot delete a running client instance. Please stop the client first.")
        for obj in self:
            if obj.state == "stopped":
                host_server, db_server = obj.saas_contract_id.plan_id.server_id.get_server_details()
                _logger.info("HOST SERER %r   DB SERVER  %r"%(host_server,db_server))
                response = client.main(domain=obj.database_name,container_id=obj.container_id,port=obj.containter_port,host_server=host_server,db_server=db_server, config_path = get_module_resource('odoo_saas_kit'))
                if not response['db_drop'] or not response['drop_container'] or not response['delete_nginx_vhost'] or not response['delete_data_dir']:
                    raise UserError("ERROR: Couldn't Drop Client Database. Please Try Again Later.\n\nOperation\tStatus\n\nDrop database: \t{}\nDrop Instance: \t{}\nDelete Domain Mapping: \t{}\nDelete Data Directory: \t{}".format(response['db_drop'], response['drop_container'], response['delete_nginx_vhost'], response['delete_data_dir']))
