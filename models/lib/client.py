...
def delete_remote_data_dir(domain,ssh_obj):
    ...
    if "odoo-server.conf" in sftp.listdir(path):#os.path.exists(path+"/odoo-server.conf"):
        try:
            if "13.0" in sftp.listdir(path+"/data-dir/addons/"):#os.path.exists(path+"/data-dir/addons/12.0"): 
                _logger.info("Permissions of Odoo addons/13.0")
                sftp.chmod(path+"/data-dir/addons/13.0",0o700)
...
def delete_data_dir(domain):
    ...
    if os.path.exists(path+"/odoo-server.conf"):
        try:
            if os.path.exists(path+"/data-dir/addons/13.0"):
                _logger.info("Permissions of Odoo addons/13.0")
                os.chmod(path+"/data-dir/addons/13.0",0o700)
            shutil.rmtree(path)
            return True
        except Exception as error:
            _logger.info("Error deleting %r data dir %r"%(domain,error))
...
def update_values(config_path):
    global nginx_vhost, data_dir, client_admin_passwd
...
def main(domain , container_id , port, host_server = None, db_server = None, config_path = None):
    ...
    response["db_drop"] = drop_db(domain,"http://{}:{}".format("localhost" if isitlocal else host_server['host'], 8888))
